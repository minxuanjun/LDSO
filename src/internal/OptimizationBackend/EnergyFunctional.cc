                ...
                for (auto r : p->residuals) // 遍历所有点的PointFrameResidual
                {
                    ...
                    for (int i = 0; i + 3 < patternNum; i += 4) {
                        ...
                        Jdelta = _mm_add_ps(Jdelta,
                                            _mm_mul_ps(_mm_load_ps(((float *) (rJ->JIdx + 1)) + i), Jp_delta_y)); // + JIy*delta_y
                        Jdelta = _mm_add_ps(Jdelta, _mm_mul_ps(_mm_load_ps(((float *) (rJ->JabF)) + i), delta_a)); //+ Ja*delta_a
                        Jdelta = _mm_add_ps(Jdelta, _mm_mul_ps(_mm_load_ps(((float *) (rJ->JabF + 1)) + i), delta_b)); // +Jb*delt_b
